...
public class CertificadoController(AppDbContext context) : ControllerBase
{
    ...
    {
        var nombreHtml = System.Net.WebUtility.HtmlEncode(nombre);
        var congresoHtml = System.Net.WebUtility.HtmlEncode(congreso);
        var colorHtml = System.Net.WebUtility.HtmlEncode(colorPrimario);
        return $$"""
        <!DOCTYPE html>
        <html lang="es">
        <head>
          ...
          <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .cert { background: #fff; width: 794px; min-height: 562px; padding: 60px 70px; border: 3px solid {{colorHtml}}; box-shadow: 0 4px 30px rgba(0,0,0,.15); position: relative; text-align: center; }
            .cert::before { content: ''; position: absolute; inset: 10px; border: 1px solid {{colorHtml}}44; pointer-events: none; }
            .header { color: {{colorHtml}}; font-size: 13px; letter-spacing: 4px; text-transform: uppercase; margin-bottom: 30px; }
            .title { font-size: 42px; font-weight: 700; color: {{colorHtml}}; margin-bottom: 20px; letter-spacing: 1px; }
            ...
            .recipient { font-size: 36px; font-weight: 700; color: #1a1a1a; border-bottom: 2px solid {{colorHtml}}; display: inline-block; padding-bottom: 6px; margin-bottom: 40px; min-width: 300px; }
            .body-text { font-size: 16px; color: #444; line-height: 1.8; margin-bottom: 16px; }
            .event-name { font-size: 22px; font-weight: 700; color: {{colorHtml}}; }
            .dates { font-size: 14px; color: #666; margin-top: 8px; }
            .footer { margin-top: 50px; font-size: 12px; color: #aaa; }
            .print-btn { position: fixed; top: 20px; right: 20px; background: {{colorHtml}}; color: #fff; border: none; padding: 10px 20px; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 600; z-index: 100; }
        ...
        <body>
          ...
          <div class="cert">
            ...
            <div class="recipient">{{nombreHtml}}</div>
            <div class="body-text">participó en el evento</div>
            <div class="event-name">{{congresoHtml}}</div>
