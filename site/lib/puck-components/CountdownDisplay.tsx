...
export default function CountdownDisplay(props: Props) {
  ...
  const toRem = (v: number, fallback: string) => (!v ? fallback : v > 4 ? `${v / 16}rem` : `${v}rem`);

  return (
    <div style={{ background: props.bgColor, padding: `${toRem(props.paddingV, '3rem')} 2rem`, textAlign: align, fontFamily: ff }}>
      {props.titulo && (
        <p style={{ fontSize: toRem(props.tituloFontSize, '1.125rem'), fontWeight: 700, color: props.tituloColor, margin: '0 0 1rem', fontFamily: ff }}>
          {props.titulo}
        </p>
      )}
      {!timeLeft ? (
        ...
        <div style={{ display: 'inline-flex', gap: '1.5rem', alignItems: 'flex-end', justifyContent: justifyMap[props.alineacion] ?? 'center', flexWrap: 'wrap' }}>
          {units.map(({ value, label }, i) => (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, minWidth: 56 }}>
              <span style={{ fontSize: toRem(props.fontSize, '3.5rem'), fontWeight: 700, color: props.color, lineHeight: 1, fontFamily: ff }}>
                {pad(value)}
              </span>
